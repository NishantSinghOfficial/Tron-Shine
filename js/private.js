...
const dividendPopData = async ()=>{
  ...
  let _pool1 = await thisTronWeb.trx.getUnconfirmedBalance(diceGame) - 0;
  let _pool2 = await thisTronWeb.trx.getUnconfirmedBalance(barsctrt) - 0;
...
const apicall = async () =>{
  ...
  .then(function(res){
    ...
    let pool = (publicString.contractBal - 0)*0.7;
